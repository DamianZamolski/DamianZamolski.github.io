...
export default function WarOfTheRingRollCalculatorPage() {
  ...
  return (
    <Page slug='war-of-the-ring-roll-calculator'>
      <form>
        <fieldset>
          <legend>{t('rolls')}</legend>
        ...
        <fieldset>
          <legend>{t('rollSuccessValue')}</legend>
        ...
        <fieldset disabled={rerolls === 0}>
          <legend>{t('rerollSuccessValue')}</legend>
          {Array.from({ length: 5 }, (_, i) => i + 2).map((value) => (
            <Fragment key={value}>
              <input
      ...
      {Object.keys(probabilities).length > 0 && (
        <>
          <p>
            {t('expectedValue')} <strong>{expectedValue.toFixed(2)}</strong>
          </p>
          <table>
            <thead>
              <tr>
