...
export default function CardSleevesCounterPage() {
  ...
  const cardSetLabel = (
    ...
    const plural = cardSet.cardTypes.length === 1 ? '' : 's';

    return `${name} (${cardSet.cardTypes.length} card type${plural})`;
  };
  ...
  const countSleeves = async () => {
    ...
    if (games.size === 0) {
      setStatus({
        kind: 'error',
        message: 'All games failed to fetch:',
        ids: failedIds,
      });
        ...
        <fieldset disabled={isFetching}>
          <label>
            ...
            <small>
              {gameIds.length === 0
                ? hasNoMatches
                  ? 'No BGG URLs detected.'
                  : 'e.g. https://boardgamegeek.com/boardgame/12345/name'
                : `${uniqueGameIds.length} unique BGG URL(s) detected.`}
            </small>
          </label>
        ...
        <output>
          {isFetching && (
            <>
              ...
              <small>{`Fetching game ${status.done}/${status.total}`}</small>
            </>
          )}
          ...
          {status.kind === 'results' && (
            <>
              ...
              {status.failedIds.length > 0 && (
                <details>
                  <summary>
                    {`${status.failedIds.length} game(s) failed to fetch`}
                  </summary>
                  <ul>
